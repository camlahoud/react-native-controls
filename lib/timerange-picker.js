...
export default class TimeRangePicker extends Component {
    constructor(props) {
        ...
        this.state = {
            modalVisible: false,
            from: moment.utc(props.value.from,"HH:mm").local().toDate(),
            to: moment.utc(props.value.to,"HH:mm").local().toDate(),
    ...
    set() {
        this.props.onValueChange({
            from: moment(this.state.from).utc().format("HH:mm"),
            to: moment(this.state.to).utc().format("HH:mm")
        });
    }
}
